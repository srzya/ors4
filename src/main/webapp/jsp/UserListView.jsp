...
<body>
 ...
 %> <label>Role :</label> <%=HTMLUtility.getList("roleId", String.valueOf(ServletUtility.getParameter("roleId", request)),
						roleList)%> &nbsp;<input type="submit" name="operation"
				...
				<table id="customers">
					...
					<tr>
						...
						<td align="center"><input type="checkbox" name="ids"
							class="case" value="<%=bean1.getId()%>"
							<%if (bean1.getRoleId() == RoleBean.ADMIN) {%> <%="disabled"%>
							<%}%>></td>
						<%
							}
						%>
						...
						<%
							if (bean1.getId() != 1) {
						%>
						<td><a href="<%=ORSView.USER_CTL%>?id=<%=bean1.getId()%>"
							id="linkID">Edit</a></td>
						<%
							}
						%>
						...
						%>
						<td><a href="" id=linkID class="isDisabled">Edit</a></td>
						<%
							}
						%>
